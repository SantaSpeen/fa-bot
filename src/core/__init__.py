...
templates = {
    ...
    "cringe": {
        "type": "HTML",
        "ints": ["", "❶", "❷", "❸", "❹", "❺", "❻", "❼", "❽", "❾", ],
        "rename": {"ВК": "Контора пидорасов (ВК)"},
        "header": "<b>֍Неделя:</b> {week_str}֎",
        "day_header": "<b>→ ̸ {sdate} ― {sname}</b>\n",
        "day_body": (
            " <b>{i}. {lesson_time}</b> - {lesson_name}\n"
            " <i>Препод:</i> {lesson_teacher}\n"
            " <i>Где?:</i> {lesson_place}\n"
        ),
        "not_ofo": [ # 1 - есть ссылка; 2 - нет ссылки
...
def render_data(data, ofo=True, template="cringe"):
    ...
    for day in data:
        ...
        s += template["day_header"].format(sdate=sdate, sname=sname.lower().capitalize())
        lessons = False
        for i, lesson in enumerate(day['lessons']):
            ...
            lesson_teacher = lesson['teacher'].lower().capitalize()
